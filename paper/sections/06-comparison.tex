...
Monte Carlo simulation (100{,}000 trials, mulberry32 PRNG seeded at 42 for reproducibility) yields $\Prob[\text{pick } \#1 \mid \text{Milwaukee}] \approx 3.4\%$ and $\E[\text{pick} \mid \text{Milwaukee}] \approx 7.21$. For comparison, Chicago (the overall pool leader) has $\Prob[\text{pick } \#1] \approx 30.1\%$ and $\E[\text{pick}] \approx 2.50$, and Memphis (Milwaukee's pool-adjacent counterpart with the same McCarty) has $\Prob[\text{pick } \#1] \approx 5.0\%$ and $\E[\text{pick}] \approx 6.30$. Countdown \cola{} runs 21 sequential bottom-up draws across nested five-team pools, and the compounding of elimination probabilities concentrates \#1-pick mass heavily on pool-1 leaders while spreading the remainder across pool-2 teams in a pattern that amplifies the effect of intra-pool ticket differences. Analytic derivation of these probabilities requires tracking $2^{21}$ branch paths; the Monte Carlo approach (applied uniformly across all 26 seasons of the backtest) is the tractable alternative. Numbers reported in this section come from \texttt{scripts/audit\_countdown\_mc.js} in the project repository, which uses a seeded PRNG so each value is reproducible to the precision shown.
...
\citet{highley2026solvable4} identifies four options for handling traded picks under \cola{}. The Substack post refers to these as ``first / second / third / fourth option''; we adopt the descriptive labels below for cross-referencing in tables and case studies:
...
\highley{We have read the Substack's ``second option'' as our Option~2 (original owner penalised) and ``third option'' as our Option~3 (receiving team penalised). On rereading, the Substack phrase ``[option two] trades both the pick and the associated priority adjustment'' could be interpreted as the priority adjustment moving \emph{with} the pick to the receiving team---which would correspond to our Option~3, not Option~2. Your stated preference is for ``option two paired with the strengthened Stepien Rule''; please confirm which of our descriptive labels matches that preference. If the mapping is reversed, both the numbering and the discussion in Section~\ref{sec:adoption} need to swap accordingly.}
...
Top team $\Prob[\#1]$ & N/A (det.) & $25.0\%$ & $14.3\%$ & $30.1\%$ & raffle by $L_i / P$ \\
